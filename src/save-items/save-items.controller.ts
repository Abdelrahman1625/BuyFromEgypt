...
export class SaveItemsController {
  ...
  async getSavedItems(@Param('entityType') entityType: string, @Param('userId') userId: string, @Query() paginationDto: PaginationDto) {
    return this.saveItemsService.getSaved(entityType as any, userId, paginationDto);
  }
}
